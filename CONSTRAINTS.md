...
| Vuln dependances | Rien >= high | `./gradlew cyclonedxBom` puis `osv-scanner scan source -r .` | CI | Externe (base de vulns). Sous high, c'est du bruit |
...
|---|---|---|
| Couverture projet (kover) | non mesuree, le premier `koverXmlReport` fera foi | ne doit pas baisser (tolerance 0,5% de derive) |
| Taille APK release | non mesuree | ne doit pas croitre |
